---
name: ww-prove-tree-identity
description: Use when a change may make a bake FASTER but not DIFFERENT (parallelising, caching, reusing, reordering): prove the whole output tree byte-identical, every file, with a non-vacuity check on measured worker counts and a refuter that must go red.
---
