...
shot() {  # shot <plane> <png>
	...
	WW_RENDER_SHOT="$(winpath "$2")" WW_RENDER_FLAT=1 WW_RENDER_VIEW=1 WW_RENDER_SIZE=360x360 \
		WW_SETTINGS_SCOPE="$(fresh_scope)" timeout 120 "$NS" --port "$PORT" "$LODL" >/dev/null 2>&1
	[ -s "$2" ]
}
rendered=""
for p in $A_PLANES; do
...
WW_RENDER_SHOT="$(winpath "$BIG")" WW_RENDER_VIEW=1 WW_RENDER_SIZE=900x900 \
	WW_SETTINGS_SCOPE="$(fresh_scope)" timeout 900 "$NS" --port "$PORT" "$LODL" >/dev/null 2>&1
if [ -s "$BIG" ]; then
	stats=$("$PY" - "$BIG" <<'PYEOF'
import sys
import numpy as np
