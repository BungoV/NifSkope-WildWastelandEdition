...
orbit() { # orbit <name> <lodm> <el> [ENV...]
	local nm="$1" d="$W/$1" lodm="$2" el="$3"; shift 3
	rm -rf "$d"; mkdir -p "$d"
	...
	if [ "$el" = 20 ]; then RP=0.13232642; RT=0.01388609; else RP=0.11702900; RT=0.00961476; fi
	REF_POP=$RP REF_TEAR=$RT $PY $ROOT/tests/spells/impostor_trunkbar.py $nm=$d@$el 2>&1 | grep -E "T1|T2|T3|TRUNK|whole"
}
