#!/bin/bash
# Syntax-check src/hkxanim.cpp with the real Makefile.Release flags, then build
# the standalone hkxanim_dump.exe (Qt6Core + Qt6Gui). Run inside MSYS2 UCRT64:
...
echo "== syntax src/hkxanim.cpp"
eval g++ -fsyntax-only $FLAGS src/hkxanim.cpp 2>&1 | grep -v "sfinae-incomplete\|qchar.h" | head -40
echo "SYNTAX-RC=${PIPESTATUS[0]}"
echo "== build release/hkxanim_dump.exe"
eval g++ -O1 $FLAGS -Wno-unused-parameter tests/hkxanim_dump.cpp src/hkxanim.cpp tests/hkxanim_shim.cpp -o release/hkxanim_dump.exe -LC:/msys64/ucrt64/lib -lQt6Core -lQt6Gui 2>&1 | grep -v "sfinae-incomplete\|qchar.h" | head -40
echo "BUILD-RC=${PIPESTATUS[0]}"
ls -l --time-style=+%H:%M:%S release/hkxanim_dump.exe 2>/dev/null
