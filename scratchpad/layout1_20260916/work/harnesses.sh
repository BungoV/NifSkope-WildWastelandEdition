...
mkdir -p "$L"
for h in lodgen_byte_gate lodgen_ladder lodgen_native lodgen_btofree lodgen_defaults \
         lodl_write lodl_water lodl_btd lodgen_terrain_vt lodgen_terrain_pbrm; do
	printf '=== %s  %s\n' "$h" "$(date +%H:%M:%S)"
