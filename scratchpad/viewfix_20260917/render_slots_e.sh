...
for L in ${LEVELS:-0 1 2 3}; do
	if tasklist | grep -qi -E "Fallout4|NifSkope"; then echo "GAME OR NIFSKOPE UP -- stop"; exit 90; fi
	WW_WINDOW_AT=1960,40 WW_LODL_OBJECTS="${LODI:-$V/urban_authored/FO4CSLOD/Commonwealth/Commonwealth.lodi}" \
	WW_LODL_SHEETS="${SHEETS:-$V/chunkC/vt/FO4CSLOD/Commonwealth}" WW_LODL_REGION="4,-12,7,-9,$L" WW_LODI_LEVEL=0 WW_LODI_SLOT=$L \
