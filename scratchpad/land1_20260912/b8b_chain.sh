...
for s in lodgen_identity lodgen_terrain lod_generation lodgen_terrain_vt lodgen_ground_cover \
	...
	echo "== $s =="
	bash "$f" > scratchpad/land1_20260912/logs/hc_$s.txt 2>&1
	rc=$?
	tail -3 scratchpad/land1_20260912/logs/hc_$s.txt | grep -E "checks|RESULT" 
	echo "   rc=$rc  segfaults=$(grep -ci 'segmentation fault' scratchpad/land1_20260912/logs/hc_$s.txt)"
done
