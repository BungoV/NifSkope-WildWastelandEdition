#!/bin/bash
# LAND1 gate B8 -- the harness chain on the FINAL exe (both parts), run through the MSYS2
...
for s in lodgen_identity lodgen_terrain lod_generation lodgen_terrain_vt lodgen_ground_cover \
         lodgen_terrain_pbrm lodgen_native lodgen_roads lodl_open animws; do
	f=tests/spells/$s.sh
	[ -f "$f" ] || { echo "== $s : NO SUCH HARNESS"; continue; }
	echo "== $s =="
	bash "$f" > scratchpad/land1_20260912/logs/hb_$s.txt 2>&1
	rc=$?
	tail -3 scratchpad/land1_20260912/logs/hb_$s.txt | grep -E "checks|RESULT" 
	echo "   rc=$rc  segfaults=$(grep -ci 'segmentation fault' scratchpad/land1_20260912/logs/hb_$s.txt)"
done
