# IMPOSTORLOOK1 section A -- bungo 2026-09-19 21:45:
#   "Do a comparison for imposters, compare each imposter with its original
#    model it was baked from, at every taken angle, side by side"
#
# One contact sheet per subject. One CELL per BAKED FRAME (all N*N bake
# directions, in the octahedral grid's own (i,j) order, labelled with the frame
# index and its azimuth/elevation). Each cell, left to right:
#
#     MESH   the original model from that bake direction (an existing grab)
#     CARD   the impostor as the viewer draws it from that same direction
#            (an existing grab)
#     FRAME  the raw baked frame cut out of the shipped *_oct_d.DDS, its
#            coverage decoded and composited on the same background
#
# OFFLINE: nothing here runs an exe. Grabs are USED WHERE THEY EXIST and never
# substituted by a nearby view -- a subject/direction with no grab gets a cell
# panel that says MISSING, and shoot_missing.sh names exactly those shots.
...
OUT  = REPO + '/scratchpad/impostorlook1_20260919/images'
...
# subject -> (fixture tag, folder holding mesh+card grabs at the BAKE
# directions, what that folder is). None = no such grabs exist anywhere.
GRABS = {
  'blast_n4': (REPO + '/scratchpad/impostorfix5_20260919/control/blast_n4_bake_ctl',
               'IMPOSTORFIX5 blast_n4_bake_ctl, blend 0, 1024x1024'),
  'blast_n8': (None, None),
  'maple_n4': (REPO + '/scratchpad/impostorfix5_20260919/control/maple_n4_bake_on',
               'IMPOSTORFIX5 maple_n4_bake_on, blend 1 (parallax ON), 1024x1024'),
  'dead_n4' : (None, None),
  'rock_n4' : (None, None),
}
MESHOF = {
...
def build(tag, maxdim=5400):
    ...
    p = '%s/A_%s_every_bake_angle.png' % (OUT, tag)
    sheet.save(p)
