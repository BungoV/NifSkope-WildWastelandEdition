...
(CONSTITUTION 8). Written incrementally.

**No other lane is alive at launch.** `ls scratchpad/*/BUILDING` printed nothing
(rc=2) at 2026-09-11, and `tasklist | grep -i -E "Fallout4|NifSkope"` printed
nothing (rc=1): the game is down and no NifSkope -- not even bungo's -- is
running. Exe on disk at launch: `release/NifSkope.exe` 2026-09-11 05:58:21,
20,855,296 B (UI5).
...
Written BEFORE any code was changed (CONSTITUTION 1: a gate invented after the
numbers are in is not a gate). Every number below is a prediction.

| id | what it measures | predicted, after | how it is read | its floor |
|---|---|---|---|---|
| **G1** | LEFT strip `LeftColumnModeSelector`: inter-segment gap | **0** | painted box edges out of the tab bar's own grab (`paintedSegment`), never `tabRect()` | UI4's separated sheet (`margin-left: 4px` on every tab) appended live -> the same predicate reads 4 and goes RED by name, then green when taken away |
| G1 | ...top / bottom / left / right-to-toolbar | 4 / 4 / 4 / 4 | the same scan | the 18:25:20 flush sheet appended live -> all four go to 0/0/0..1/3 |
| G1 | ...segment height in the row | 27 in a row of 35 | the same scan | a degenerate box (w < 20 or h < 8) refuses the whole measurement |
| **G2** | RIGHT strip `LodPanelModeSelector`: inter-segment gap | **0** | the same painted-box scan, on the LOD strip (L8's missing half) | the same separated sheet appended to the LOD strip -> 4, red |
| G2 | ...top / bottom, segment height | 4 / 4, h 27 in a row of 35 | L8 as shipped | the LEFT strip must read the same two numbers |
| **G3** | no `QDockWidget` named `TimelineDock` exists; `skope->timeline` is null; the Workspaces menu's "Animation" entry opens `AnimWorkspaceDock` | 0 / null / yes | `findChild<QDockWidget*>("TimelineDock")`, the menu dump in the log | the same scans DO find `AnimWorkspaceDock` and "LOD Generation" |
| G3 | `animws.sh` | 57 / 0, 1 skip | the spell | unchanged baseline |
| **G4** | unstamped numeric spin boxes in the new Animation dock | **0** of 11 | `QAbstractSpinBox::property("wwScrubbed")` over `findChildren` | a deliberately un-stamped `QDoubleSpinBox` made inside the harness and parented to the dock is counted as **1** in the same run, then destroyed |
| G4 | every spin box's stepper (up/down) rect inside its own rect and >= 8 px wide | 11 of 11, none clipped | `QStyle::subControlRect( CC_SpinBox, SC_SpinBoxUp/Down )` against `rect()` | the old 0.050 / 0.1000 clip reproduced live by pinning a box to the old strip's width, which must report a clipped stepper |
| **G5** | the dock's visible binding label | **<= 40 chars, both numbers** (e.g. `78 of 95 bones`) | `AnimWsNote`'s `text()` | the full paragraph is still reachable, in the label's TOOLTIP, and is > 40 chars |
| **G6** | every viewport-header menu button: first arrow-ink column minus last icon/label-ink column | **>= 2 px** | two grabs of the same button, one with the menu-indicator suppressed; the changed columns ARE the arrow | with the arrow's air taken away live (the UI3 sheet), the same read gives **< 2** on the two icon buttons, named in the log |
| **G7** | exe newer than every path in `git status --porcelain -- src res tools tests`; `cmp res/style.qss release/style.qss`; the rung | no STALE line; silent; `release/NifSkope.before_ui6.exe` == 20,855,296 B | the build-verify sweep | -- |
| **G8** | no NifSkope left running by this lane | 0 | `tasklist` | -- |

Harness baselines this lane must not move except where it says so, by check name:
`water_ui` 76/0 floor 62, `ui_align` 11/0, `top_bar` 43/5, `files_tab` 28/2,
`animws` 57/0 (1 skip), `hkxanim_ui` 48/1, `loaded_nifs` 166/0.

**Predicted movements, registered here rather than explained afterwards:**

1. `water_ui.sh` gains group **A** (the arrows) and two checks in group S
   (the inter-segment gap's own floor), and group R4's expectation is amended
   because the skin's button sheet now carries a `padding-right` on the
   menu-arrow selector alone. Floor rises from 62.
2. `water_ui.sh`'s group L gains L8's missing half (2 checks).
3. `hkxanim_ui.sh` is RE-AIMED at the new Animation dock, because the dock it
   measured is being retired. Its letters (a)..(g) keep their meanings; the
   widget names change. Count will move and every moved check is named in
   section 3.

---

## 1. Case 0 and the old manager's inventory

### 1.1 The probe (skill `ww-qss-geometry-probe`)

`scratchpad/ui6_20260910/probe.cpp`, built by `build_probe.sh`, output
`scratchpad/ui6_20260910/probe_out.txt`. It links Qt only, runs
`-platform offscreen`, and cannot touch `release/NifSkope.exe`.

**Part 1, the strip. Case 0 reproduces the shipped window exactly**, which is
what licenses everything after it -- the same five numbers `water_ui.sh` read
on the real 20:45:47 / 05:58:21 exe:

```
  case 0  (the sheet that shipped)   top 4  bottom 4  left 4  right 4  between 4 / 4   segment h 27
  case 1  (air 0, the 18:25:20 strip) top 0  bottom 0  left 0  right 3  between 0 / 0   segment h 35
  case J4 (THE CANDIDATE)             top 4  bottom 4  left 4  right 4  between 0 / 0   segment h 27
  case J0 (the candidate at air 0)    top 0  bottom 0  left 0  right 3  between 0 / 0   segment h 35
```

Two things it settles:

1. **J4 is the whole of bungo's ruling in one sheet**: the four outer distances
   keep UI4's 4 px, the segments touch, and the segment is still 27 px tall in a
   35 px row. Nothing else moves -- `tabRect()` reads `128 / 127 / 128 at y 0,
   h 35` in every one of the four cases, which is why the gate reads pixels.
2. **J0 is case 1, number for number**, and the strings are byte-identical too:
   the new sheet's `MARGINFIRST` token carries its own leading space, so at
   `UI/SegmentedStripAir = 0` the emitted stylesheet is the 18:25:20 one to the
   byte and not one space off it. The way back stays exact (CONSTITUTION 7).

**Part 2, the arrow. Case A0 reproduces the DEFECT**, which is the same test in
the other direction -- on the icon-only buttons the arrow starts on the column
after the glyph's last:

```
  padding-right      A0 (4, res/style.qss's own)   6    8    10
  "Global" (text)                   gap 49         51   53    55
  pivot dot                         gap  3          5    7     9
  snap                              gap  0          2    4     6
  grid                              gap  0          2    4     6
```

The arrow is 9 columns wide and sits against the padding box's right edge, so
every pixel of `padding-right` above res/style.qss's own 4 is a pixel of clear
background. **8 is the value taken**: 4 px of clear background on the tightest
button in the row, the middle of the usable range rather than the 2 the gate
demands. The button grows by the same 4 px; nothing vertical moves.

It also settled the instrument. `QWidget::grab()` on an auto-raise QToolButton
returns a pixmap Qt filled with **white** -- background `#efefef`, glyph
`#e6e8eb`, seven levels apart, so the first run of the sweep reported
"UNREADABLE" on every icon button. Rendering over a known dark fill
(`QWidget::render` with `DrawChildren` onto a pixmap filled with `bgBar`) is
what makes the ink findable, and the in-application gate uses the same method.

### 1.2 The old Animation Manager's inventory

Everything the retired dock offered, and whether the new Animation dock has it.
**Nothing was ported. This table is the director's and bungo's to act on.**

| the old Animation Manager had | the new Animation dock |
|---|---|
| a sequence selector (the NIF's `NiControllerSequence` blocks) | YES -- one list, sequences and loaded `.hkx` clips together |
| transport: to start / prev key / play back / play / next key / to end | YES, plus a Stop button |
| Loop and Switch mirrored from the render toolbar | YES |
| playback speed | YES |
| load / unload a `.hkx`, root motion | YES |
| auto-key from the viewport gizmo | YES |
| isolate (solo) a block from the dock | YES (the signal is connected) |
| a time field in seconds, and a seconds/frames toggle | PARTIAL -- the field is FRAMES at the clip's own rate and the readout prints the seconds beside it; there is no toggle |
| per-interpolator lanes for EVERY controller in the file (alpha, colour, visibility, UV...) | **NO** -- the dope sheet's rows are bone tracks; non-transform controllers have no row |
| editing the NIF's OWN keys: insert, delete, duplicate, copy/paste, scale, nudge, easing (flatten / smooth / linearize / ease in / ease out), set interpolation, clear a channel, mute a lane, add a text-key marker | **NO** -- the new dock edits the `.hkx` clip document; a NIF sequence is shown READ-ONLY, with the words "edit them in the Blocks tab" |
| a graph (F-curve) view with tangents | **NO** |
| a key inspector (value / tangent / interpolation of the selected key) | **NO** |
| channel copy/paste between lanes | **NO** -- and a spell dialog still tells the user to use it (see section 5) |
| CSV export / import of a lane | **NO** |
| a lint scan of the file's animation data | **NO** |
| a lane filter box | **NO** -- neither the list nor the sheet has a filter |
| snap toggle + snap time step + snap value step | **NO** -- the sheet snaps to whole frames always. (These were the two spin boxes whose steppers bungo photographed.) |
| normalise the graph's values | **NO** (there is no graph) |
| follow the playhead | **NO** |
| auto-isolate: filter the lanes by the selected geometry | **NO** |
| text keys shown as ruler markers | PARTIAL -- the sheet's marker row is the `.hkx` clip's ANNOTATIONS; a NIF `NiTextKeyExtraData` has no marker row |
| a physics recording driving the timeline's range (`GLView`) | **NOT MEASURED** -- the route exists in `glview.cpp` and pointed at the retired widget; nothing here tested whether the Animation dock picks it up |

The seven in bold are what a user loses today. The largest by far is **editing
a NIF's own animation keys**: the new dock is a Havok clip editor, and the NIF
side of it is read-only.
