#!/bin/bash
# lane BUILD10 -- the WATER4 harness chain, one NifSkope instance at a time.
set -u
ROOT=/e/Projects/NifskopeWildWastelandEdition
OUT=$ROOT/scratchpad/water4_20260910
cd "$ROOT" || exit 2
mkdir -p "$OUT"
...
run () {
	...
	grep -aE "checks, [0-9]+ failures|^PASS|^FAIL|PASS$|FAIL \(|flow gates green" "$OUT/gate_$label.txt" | tail -6
	echo
}
