#!/bin/bash
# GRADE1 bake helper -- TILING4's t4_bake.sh with this lane's out-dir and
# nothing else changed, so an arm of the byte-identity gate is the SAME command
# line with one switch added.  Bakes go into THIS lane's out-dir ONLY; never his
# installed Data\Terrain, never the whole Commonwealth unasked.
#
#   usage: g_bake.sh <exe> <variant> <tile> [extra lodgen args...]
#   t2024 = (-20,24)..(-17,27)    t2020 = (-20,20)..(-17,23)
...
TAG="${TILE//[:,]/_}"
OUT="$R/scratchpad/grade1_20260911/out/$VAR/$TAG"
