...
# BAKE2 copy of EXTENT1 composite.py: the census reads env LODI (any worldspace).
# usage: LODI=<.lodi> python composite.py <T.png> <A.png|-> <B.png|-> <out.png> <x0> <y0> <x1> <y1> <ortho half-width> [nocensus]
import sys
import numpy as np
...
if len(sys.argv) > 10 and sys.argv[10] == 'nocensus':   # an oblique camera: the top-down cell map does not apply
    sys.exit(0)
sys.path.insert(0, r'E:/Projects/NifskopeWWE-bake2/scratchpad/bake2_20260925')
import os, checks
cells = checks.placement_cells(os.environ['LODI'])[0]; ninst = sum(cells.values())   # BAKE2: the worldspace's .lodi from env LODI
...
for (x, y), n in sorted(out):
    print('   no-terrain cell %d,%d: %d placements' % (x, y, n))
