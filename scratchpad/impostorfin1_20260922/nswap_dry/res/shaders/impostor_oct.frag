...
uniform sampler2D NormalSheet;     // _n       : R nX, G nY, B sway, A height (layout 2)
...
void main()
{
	...
	for ( int k = 0; k < 3; k++ ) {
		...
		if ( useHeightBlend ) {
			...
			float h = textureLod( NormalSheet, uv, 0.0 ).a;	// layout 2: height in A
		...
		if ( swayAmplitude != 0.0 ) {
			float sw = textureLod( NormalSheet, uv, 0.0 ).b;	// layout 2: sway in B
		...
		height     += n.a * wc;	// layout 2: height in A
		sway       += n.b * wc;	// layout 2: sway in B
		if ( hasMaskSheet )
	...
	for ( int i = 0; i < 3; i++ ) {
		...
		if ( dot( L, L ) < 1e-8 )
			continue;
		/* Two-sided, and deliberately so: a card's baked normal faces the
		 * frame it came from, and a leaf lit from behind at LOD distance
		 * reading black is a worse lie than a leaf lit from both sides. */
		float ndl = abs( dot( normal, normalize( L ) ) );
		lit += sqrt( max( lightSourceDiffuse[i].rgb, vec3( 0.0 ) ) ) * ndl;
	}
