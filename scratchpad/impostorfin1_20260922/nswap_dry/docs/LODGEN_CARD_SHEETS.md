...
| normal A (height) | window depth of the orthographic bake. **0.5 is the card plane**; `units = (B − 0.5) × depthSpan`, with `depthSpan = 3 × max(boundRadius, 1024)`. Drives pixel depth offset, ghost-free frame blending, shadows and the model-to-card transition |
| normal B (sway) | `h² × (0.35 + 0.65·r)`, `h` up from the view's own bottom row, `r` the radius from the silhouette's axis; **explicit 0 for rigid objects** |
