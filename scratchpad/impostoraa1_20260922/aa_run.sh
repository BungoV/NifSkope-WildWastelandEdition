#!/bin/sh
# IMPOSTORAA1 copy of IMPOSTORFIN1's fin1_run.sh (NS= and SET= overridable) -- the bakes and the grabs, strictly serial, one harness NifSkope
...
MINE="$REPO/scratchpad/impostoraa1_20260922"
...
bake)
	guard
	echo "exe $NS sha1 $(sha1sum "$NS" | cut -c1-8)"
	for t in ${TAGS:-blast_n4 blast_n8 maple_n4 dead_n4 rock_n4}; do
		echo "== bake $t 512"; bake "$t" 512 "$MINE/$SET/$t"
		echo "  aa line: $(grep -E '^aa ' "$MINE/$SET/$t/bake/"*.txt || echo none)"
	done ;;
shots)
	guard
	shots "$2" "$3" "$4" ;;
