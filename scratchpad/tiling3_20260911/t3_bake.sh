#!/bin/bash
# TILING3 bake helper -- TILING2's t2_bake.sh with one addition: a REGION
# argument, so the vanilla-reuse census can be counted over ground wide enough
# to contain both classes of chunk.  Bakes go into THIS lane's out-dir ONLY;
# never his installed Data\Terrain, never the whole Commonwealth unasked.
#
#   usage: t3_bake.sh <exe> <variant> <tile> [extra lodgen args...]
#   t2024 = (-20,24)..(-17,27)    t2020 = (-20,20)..(-17,23)   -- TILING2's two
#   edgeN = (-36,20)..(-21,35)    a 4x4-chunk block at the north-west edge
#   r:X0,Y0,X1,Y1                 any cell rectangle, for the census
#
...
# a rectangle spelled "r:X0,Y0,X1,Y1" cannot be a Windows directory name
TAG="${TILE//[:,]/_}"
OUT="$R/scratchpad/tiling3_20260911/out/$VAR/$TAG"
